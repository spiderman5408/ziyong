...
Download_aria2_conf(){
	mkdir "${file}" && cd "${file}"
	wget --no-check-certificate -N "https://raw.githubusercontent.com/643822883/daige/master/aria2/aria2.conf"
	[[ ! -s "aria2.conf" ]] && echo -e "${Error} Aria2 配置文件下载失败 !" && rm -rf "${file}" && exit 1
	wget --no-check-certificate -N "https://raw.githubusercontent.com/643822883/daige/master/aria2/dht.dat"
	[[ ! -s "dht.dat" ]] && echo -e "${Error} Aria2 DHT文件下载失败 !" && rm -rf "${file}" && exit 1
	echo '' > aria2.session
}
Service_aria2(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/643822883/daige/master/aria2/aria2_centos -O /etc/init.d/aria2; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/643822883/daige/master/aria2/aria2_debian -O /etc/init.d/aria2; then
			echo -e "${Error} Aria2服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/aria2
...
Set_aria2_RPC_passwd(){
	...
	stty erase '^H' && read -p "(默认密码: laog 密码请不要包含等号 = 和井号 #):" aria2_RPC_passwd
	echo
