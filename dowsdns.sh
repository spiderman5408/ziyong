...
Download_dowsdns(){
	cd "/usr/local"
	wget -N --no-check-certificate "https://raw.githubusercontent.com/643822883/daige/master/downdns/dowsDNS.zip"
...
Service_dowsdns(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/643822883/daige/master/downdns/dowsdns_centos" -O /etc/init.d/dowsdns; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/643822883/daige/master/downdns/dowsdns_debian" -O /etc/init.d/dowsdns; then
			echo -e "${Error} DowsDNS 服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/dowsdns
